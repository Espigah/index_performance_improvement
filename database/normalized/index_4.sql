...
include (product_id, brand_id)
where product_name = 'sKhKNXAjVETXggXsaJmGVNsLMcVJqhMRXJCDQSshPSDSvixWnAtEnPUBxsXRUeUqFGznNqeZbAIvTKxWsbpPoGweXFqAyBtwUcKd'
;
...
include (purchase_id)
where purchase_detail_status_id = 1
;

-- drop index normalized.brand_name_idx;
CREATE INDEX  IF NOT EXISTS brand_name_idx ON "normalized".brand (brand_name) 
include (brand_id)
where brand_name = 'iZfrTQElElEhPPeIdtiRFvXmRNkJeQrzAoQuTAtirWPuwrZNmptbVSrbHTYxYhrmjlClDtvfIZIiISYT'
;
...
include (purchase_id, product_id)
where purchase_detail_status_id = 1
;
